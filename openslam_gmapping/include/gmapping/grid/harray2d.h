...
namespace GMapping
{
    ...
    /**
     * @brief Map 类似金字塔的形式组织地图内存类
     *
     * Array2D<autoptr<Array2D<Cell>>> 本身是一个Array2D，其存储单元是一个指向Array2D的智能指针autoptr
     *
     * @param Cell 地图单元类型
     */
    template <class Cell>
    ...
    protected:
        ...
        PointSet m_activeArea; /* 有效面积 */
        int m_patchMagnitude;  /* 块大小 2^n */
        int m_patchSize;       /* 块大小 n m_patchSize = 2^m_patchMagnitude */
    };
    ...
    IntPoint HierarchicalArray2D<Cell>::patchIndexes(int x, int y) const
    {
        /* 判断范围 */
        if (x >= 0 && y >= 0)
            /* 在patchIndexes函数中将给定的坐标值右移了m_patchMagnitude位，这相当于除以了2^n
               在计算机中除法的计算是很慢的，所以这里通过移位的方式替代除法操作 */
    ...
    Cell &HierarchicalArray2D<Cell>::cell(int x, int y)
    {
        /* 在HierarchicalArray2D中把地图分为若干个patch，其拆分的粒度由m_patchMagnitude来描述
           在cell函数中首先通过成员函数patchIndexes计算给定的坐标所属的patch的索引 */
        IntPoint c = patchIndexes(x, y);
        assert(this->isInside(c.x, c.y));

        /* 判定对应的patch是否分配过内存，若未分配过则分配
           可以在一定程度上减少内存的使用，只在需要的时候才申请新的内存 */
